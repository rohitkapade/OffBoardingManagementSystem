...
public class DepartHeadServiceImple implements DepartHeadService{
	...
	public ResignationApplicationResponseDTO updateStatusOfApplication(ApplicationUpdateByDhDto applicationUpdateByDh) throws ResignationNotFoundException, NotAllowedException {
		...
		if(resigApplication.getNextStep().equals(Status.SCHEDULE_EXIT_INTERVIEW) &&  deptHeadRepo.findById(applicationUpdateByDh.getApplicationId()).isPresent()) {
			throw new NotAllowedException("Scheduling interview can only be approved by HR manager");
		}
		else if(resigApplication.getNextStep().equals(Status.KNOWLEDGE_TRANSFER) && deptHeadRepo.findById(applicationUpdateByDh.getApplicationId()).isPresent()) {
		...
		else if(resigApplication.getNextStep().equals(Status.REVOKE_COMPANY_ACCESS) && deptHeadRepo.findById(applicationUpdateByDh.getApplicationId()).isPresent()) {
			throw new NotAllowedException("Revoking access can only be approved by HR manager");
		}
		else if(resigApplication.getNextStep().equals(Status.UPDATE_HR_RECORDS) &&  deptHeadRepo.findById(applicationUpdateByDh.getApplicationId()).isPresent()) {
			throw new NotAllowedException("Updating HR records can only be approved by HR manager");
		}
		else if(resigApplication.getNextStep().equals(Status.RETRIEVE_COMPANY_PROPERTY) &&  deptHeadRepo.findById(applicationUpdateByDh.getApplicationId()).isPresent()) {
			throw new NotAllowedException("Retriving company can only be approved by HR manager");

		}
		else if(resigApplication.getNextStep().equals(Status.CONDUCT_EXIT_INTERVIEW) &&  deptHeadRepo.findById(applicationUpdateByDh.getApplicationId()).isPresent()) {
			throw new NotAllowedException("Conducting exit interview can only be approved by HR manager");

		}
		else if(resigApplication.getNextStep().equals(Status.NOTIFIY_ALL_EMPLOYEE) &&  deptHeadRepo.findById(applicationUpdateByDh.getApplicationId()).isPresent()) {
